...
玉川大学 工学部「オペレーティングシステム」第1回〜第6回の復習クイズ（静的フロントエンド）。
学生がブラウザで全50問を解き、結果を裏側で Google フォームへ自動送信します。

## 特長

- **全50問**（第1〜6回の講義スライドを出典に作成、各問に解説・出典付き）
...
`js/config.js` に集約。対象フォームの `FB_PUBLIC_LOAD_DATA_` を解析して確定し、
実送信（HTTP 200）で検証済みの値を使用。

| 項目 | パラメータ |
| --- | --- |
| 学籍番号 | `entry.1839097959`（数字のみ） |
| 氏名 | `entry.41642044` |
| クイズ結果データ | `entry.223334294`（結果JSONを格納。メールもこの中に含む） |
...
### 結果データ（`entry.223334294`）のJSON形式（例）
